...
int main(int argc, char** argv) {
    ...
    // load camera intrinsic (to project image point into unit plane)
    ...
    for (size_t i = 0; i < vvKps.size(); i++)
    {
        ...
        // left_kps.at<float>(i, 0) = (vvKps.at(i)[0] - K.at<float>(0, 2)) / K.at<float>(0, 0);
        // left_kps.at<float>(i, 1) = (vvKps.at(i)[1] - K.at<float>(1, 2)) / K.at<float>(1, 1);
        // right_kps.at<float>(i, 0) = (vvKps.at(i)[2] - K.at<float>(0, 2)) / K.at<float>(0, 0);
        // right_kps.at<float>(i, 1) = (vvKps.at(i)[3] - K.at<float>(1, 2)) / K.at<float>(1, 1);
    ...
    // project 2d image points into unit plane

    // triangulation
    Mat points_4d;
    // cout << "kps.size: " << left_kps.size() << ", " << right_kps.size();

    cv::triangulatePoints(T1, T2, left_kps, right_kps, points_4d);
    cout << "point num: " << points_4d.size() << endl;

    Mat p_tmp;
    vector<Point3d> points;
    for (int i = 0; i < points_4d.cols; i++)
    {
        // cout << "points_3d: " << points_4d.at<float>(i) << endl;
